...
_.mixin({
    ...
    toKb: function (num) {
        ...
        if (i > 0)
            return this.format("{0}{1}", this.numberFormat(_val), _suffixes[i]);
        return num;
    ...
    numberFormat: function (num, padding) {
        if (this.isNullOrUndefined(num))
            return 0;

        if (!padding)
            padding = 0;

        return parseFloat(num).toFixed(padding).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
    },
